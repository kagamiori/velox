...
class ApproxPercentileResultVerifier : public ResultVerifier {
 ...
 private:
  ...
  // is the rank of the first appearance of 'value' when values in the current
  // frame are sorted by sortingKeysAndOrders and 'order_max' is the rank of the
  // last appearance. For example, for a table 't(c0, c1, c2, weight, row_num)'
  // and a window operation 'approx_percentile(c0, percentile) over (partition
  // by c1 order by c2 desc rows between 1 preceding and 1 following)', this
  // method essentially returns the result of the following query:
  // SELECT
  //      c1,
  //      row_num,
  //      NULL AS actual,
  ...
  RowVectorPtr computePercentilesForWindow(
    ...
    planBuilder
        ...
        .singleAggregation(
            append(groupingKeys_, {"row_number", "bucket_k"}),
            {"sum(bucket_v) as weight"})
        .window(
            {fmt::format(
